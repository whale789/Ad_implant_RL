...
from Env_Small import Ad_Environment
...
def main():
    ...
    ad_limit_list=[[0.445,0.465,0.03,0.23],[0.442,0.465,0.08,0.3],[0.435,0.55,0.03,0.1],[0.43,0.53,0.035,0.055],[0.125,0.52,0.05,0.12],[0.565,0.52,0.1,0.3],[0.282,0.4,0.05,0.14]]
    density_layer=0
    env=Ad_Environment(ad_limit_list[0][0],ad_limit_list[0][1],layer,ad_counter,ad_limit_list[0][2]/2,ad_limit_list[0][3]/2,ad_limit_list[0][0],ad_limit_list[0][1],ad_limit_list[0][2],ad_limit_list[0][3],
                       total_step=100,ad_density=0)
    max_reward=float('-inf')
    j=0
    for i in range(episodes):
        ad_limit_x=ad_limit_list[j][0]
        ad_limit_y=ad_limit_list[j][1]
        ad_limit_width=ad_limit_list[j][2]
        ad_limit_height=ad_limit_list[j][3]
        ad_width=ad_limit_list[j][2]/3
        ad_heigth=ad_limit_list[j][3]/3
        ad_state_x=ad_limit_list[j][0]
        ad_state_y=ad_limit_list[j][1]
        j=(j+1)%6-1
        if ad_state_x+ad_width/2<ad_limit_x+ad_limit_width/2 and ad_state_x-ad_width/2>ad_limit_x-ad_limit_width/2 and ad_state_y+ad_heigth<ad_limit_y+ad_limit_height and ad_state_y-ad_heigth>ad_limit_y-ad_limit_height:
            ...
            if episode_reward_sum>max_reward:
                ...
                torch.save(model.state_dict(), "best_model_small2.pth")
        else:
            i=i-1
            continue
def test():
    ...
    dqn = DQN()
    # model=DQNNet()
    DQNNet().load_state_dict(torch.load('best_model_small2.pth'))
    # model.eval()
    #(0.43,0.58) (0.46,0.35)
    ...
    ad_limit_list = [[0.445, 0.465, 0.03, 0.23], [0.442, 0.465, 0.08, 0.3], [0.435, 0.55, 0.03, 0.1],
                     [0.43, 0.55, 0.08, 0.25], [0.125, 0.52, 0.05, 0.12], [0.565, 0.52, 0.1, 0.3],
                     [0.282, 0.4, 0.05, 0.14]]
    ad_state_list=[]
    for i in range(len(ad_limit_list)):
        ad_state_x = ad_limit_list[i][0]
        ad_state_y = ad_limit_list[i][1]
        ad_width = ad_limit_list[i][2]/3
        ad_heigth = ad_limit_list[i][3]/3
        ad_state_list.append([ad_state_x,ad_state_y,ad_width,ad_heigth])
    ...
    for i in range(len(ad_limit_list)):
        ...
        ep_reward=0
        finally_state = []
        ad_limit_x = ad_limit_list[i][0]
        ad_limit_y = ad_limit_list[i][1]
        ad_limit_width = ad_limit_list[i][2]
        ad_limit_height = ad_limit_list[i][3]
        ad_width = ad_limit_list[i][2] / 2
        ad_heigth = ad_limit_list[i][3] / 2
        ad_state_x = ad_limit_list[i][0]
        ad_state_y = ad_limit_list[i][1]
        env = Ad_Environment(ad_state_x, ad_state_y, layer, ad_counter, ad_width, ad_heigth, ad_limit_x, ad_limit_y,
                             ad_limit_width, ad_limit_height,
        ...
        image_path = f"Datas/Testing_scenarios/ad{i+1}.png"
        output_path = f"ad{i+1}_test.jpg"
